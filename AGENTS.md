...
- **Environment Variables:**
  ...
  - `PERSONA_ID`: ID of the "Virginia" persona (generated by `backend/create_persona.js`).
  - `CALLBACK_URL`: Public URL for the webhook (e.g., via ngrok).
  - `PORT`: Backend port (default 3001).

## 5. Development Workflow
1.  **Database Setup:**
    - Create DB: `createdb spanish_tutor`
    - Init Schema: `cd backend && node init_db.js`
2.  **Persona Setup:**
    - Run `cd backend && node create_persona.js` to generate a `PERSONA_ID`. Add this to `.env`.
3.  **Start Backend:** `cd backend && npm run dev` (Port 3001)
4.  **Start Frontend:** `cd frontend && npm run dev` (Port 5173)
5.  **Expose Webhook:**
    - Run `ngrok http 3001`
    - Copy the HTTPS URL and add `/webhook` (e.g., `https://xxxx.ngrok-free.app/webhook`) to `.env` as `CALLBACK_URL`.
6.  **Debugging:** Use the "Debug: Trigger Confusion" button in the UI to simulate Tavus perception events.
